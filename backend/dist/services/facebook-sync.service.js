...
export class FacebookSyncService {
     ...
     */
    async syncChannelConversations(channel) {
        try {
            ...
            console.log(`📱 Syncing channel: ${channel.meta?.pageName || channel.id}`);
            ...
            const response = await axios.get(`https://graph.facebook.com/v18.0/me/conversations`, {
                params: {
     ...
     */
    async processConversation(conversation, channel, pageAccessToken) {
        try {
            // Find the user participant (not the page)
            const pageId = channel.externalId;
            const userParticipant = conversation.participants.data.find((p) => p.id !== pageId);
            if (!userParticipant) {
            ...
            if (userParticipant.picture?.data?.url) {
                ...
                try {
                    const profileResponse = await axios.get(`https://graph.facebook.com/${userId}`, {
                        params: {
                    ...
                    try {
                        const fallbackResponse = await axios.get(`https://graph.facebook.com/v18.0/${userId}`, {
                            params: {
            ...
            if (!userContact) {
                ...
                const currentFields = userContact.fields;
            ...
            if (!thread) {
                thread = await prisma.inboxThread.create({
                    data: {
                        ...
                        unreadCount: sortedMessages.filter((m) => m.from?.id !== pageId).length,
                        status: 'open',
                    },
                });
...
//# sourceMappingURL=facebook-sync.service.js.map