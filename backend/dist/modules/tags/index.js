...
const tagsModule = async (fastify) => {
    // ============================================
    // TAG CATEGORIES
    // ============================================
    ...
    fastify.post('/categories', async (request, reply) => {
        const { workspaceId, name, color } = request.body;
        if (!workspaceId || !name) {
    ...
    fastify.post('/', async (request, reply) => {
        const { workspaceId, name, categoryId, color } = request.body;
        if (!workspaceId || !name) {
            return reply.status(400).send({ success: false, error: 'workspaceId and name are required' });
        }
        try {
    ...
    fastify.put('/:id', async (request, reply) => {
        const { id } = request.params;
        const { name, categoryId, color } = request.body;
        try {
