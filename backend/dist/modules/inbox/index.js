...
export default async function (fastify) {
    ...
    fastify.get('/threads', async () => {
        try {
            ...
            const threads = dbThreads.map(thread => {
                const userFields = thread.user.fields;
                return {
                    id: thread.id,
    ...
    fastify.get('/threads/:id', async (request) => {
        try {
            ...
            const typingIndicators = fastify.typingIndicators;
    ...
    // Mark thread as read
    fastify.post('/:id/mark-read', async (request) => {
        try {
            const { id } = request.params;
    ...
    fastify.post('/:id/reply', async (request, reply) => {
        try {
            ...
            const pageAccessToken = thread.channel.tokens.pageAccessToken;
            if (!pageAccessToken) {
                return {
                    success: false,
            ...
            if (fileData) {
                ...
                const fileValidation = validateFileForChannel(channelType, fileType, fileData.length, fileName);
            ...
            try {
                ...
                if (fileData) {
                    ...
                    sendResponse = await axios.post('https://graph.facebook.com/v18.0/me/messages', formData, {
                        params: {
                    ...
                    if (messageText) {
                        const textResponse = await axios.post('https://graph.facebook.com/v18.0/me/messages', {
                    ...
                    sendResponse = await axios.post('https://graph.facebook.com/v18.0/me/messages', {
                        recipient: { id: thread.user.externalId },
                        message: { text: messageText },
                        messaging_type: 'RESPONSE',
                    }, {
