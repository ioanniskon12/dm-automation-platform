...
// MESSENGER TRIGGERS
// ============================================
export const MESSENGER_TRIGGERS = [
    {
        ...
        matches: (event, config) => {
            ...
            if (event.postId === config.postId) {
                ...
                if (config.keywords && config.keywords.length > 0) {
                    ...
                    return config.keywords.some((keyword) => commentText.includes(keyword.toLowerCase()));
                }
                return true;
            }
            return false;
...
// INSTAGRAM TRIGGERS
// ============================================
export const INSTAGRAM_TRIGGERS = [
    {
...
export function getTriggersByChannel(channel) {
    return ALL_TRIGGERS.filter(trigger => trigger.channel === channel || trigger.channel === 'all');
}
export function validateTriggerConfig(triggerTypeId, config) {
...
export function matchesTrigger(triggerTypeId, event, triggerConfig) {
    const triggerType = getTriggerType(triggerTypeId);
