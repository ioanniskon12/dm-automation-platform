...
const campaignsModule: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', async (request, reply) => {
    const { workspaceId, type, status, channelId } = request.query as {
      workspaceId?: string;
      type?: string;
      status?: string;
      channelId?: string;
  ...
  fastify.post('/:id/activate', async (request, reply) => {
    ...
    try {
      ...
      return {
        ...
        campaign: {
          ...updated,
          typeInfo: CAMPAIGN_TYPES[updated.type as keyof typeof CAMPAIGN_TYPES],
  ...
  fastify.delete('/:id/subscribers/:subscriberId', async (request, reply) => {
    const { subscriberId } = request.params as { id: string; subscriberId: string };
