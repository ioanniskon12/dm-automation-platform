...
import {
  ALL_TRIGGERS,
  getTriggerType,
  getTriggersByChannel,
  validateTriggerConfig,
  matchesTrigger
} from '../../config/trigger-types.js';

export default async function (fastify: any) {
  // Get all available trigger types (for dropdown/selection in UI)
  fastify.get('/types', async (request: any) => {
  ...
  fastify.get('/types/:id', async (request: any) => {
  ...
  fastify.post('/:id/toggle', async (request: any) => {
    try {
      const { id } = request.params;
  ...
  fastify.post('/match', async (request: any) => {
    try {
