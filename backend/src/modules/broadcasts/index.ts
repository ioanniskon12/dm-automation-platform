...
export default async function (fastify: any) {
  ...
  fastify.get('/', async (request: any) => {
    ...
    try {
      ...
      const statsWhere: any = { workspaceId: workspaceId || undefined };
      if (channel && channel !== 'all') {
  ...
  fastify.put('/:id', async (request: any) => {
    ...
    try {
      ...
      if (contentBlocks !== undefined) updateData.contentJson = JSON.stringify(contentBlocks);
      if (audienceFilters !== undefined) updateData.audienceFilterJson = JSON.stringify(audienceFilters);
      if (audienceEstimate !== undefined) updateData.audienceEstimate = audienceEstimate;
  ...
  // Duplicate broadcast
  fastify.post('/:id/duplicate', async (request: any) => {
    const { id } = request.params;
  ...
  fastify.post('/audience-preview', async (request: any) => {
    ...
    try {
      ...
      if (filters) {
        ...
        if (filters.lastInteraction) {
          const now = new Date();
          let hours = 24;
          if (filters.lastInteraction === '24h') hours = 24;
          else if (filters.lastInteraction === '7d') hours = 24 * 7;
          else if (filters.lastInteraction === '30d') hours = 24 * 30;
  ...
  fastify.post('/:id/send', async (request: any) => {
    const { id } = request.params;
  ...
  // Schedule broadcast
  fastify.post('/:id/schedule', async (request: any) => {
    const { id } = request.params;
  ...
  fastify.post('/:id/cancel', async (request: any) => {
    const { id } = request.params;
...
// Handle "Stop" keyword - unsubscribe user from broadcasts
  fastify.post('/unsubscribe', async (request: any) => {
  ...
  fastify.post('/resubscribe', async (request: any) => {
  ...
  fastify.get('/:id/report', async (request: any) => {
    const { id } = request.params;
    ...
    try {
      ...
      const statusCounts = broadcast.recipientLogs.reduce((acc: any, log) => {
