...
export class ComplianceEngine {
  ...
  private messageCounts: Map<string, { count: number; resetAt: Date }> = new Map();
   ...
   */
  private async checkInstagramPolicy(
    context: PolicyContext,
    messageType: string
  ): Promise<PolicyDecision> {
   ...
   */
  private async checkMessengerPolicy(
    context: PolicyContext,
    messageType: string
  ): Promise<PolicyDecision> {
   ...
   */
  private async checkWhatsAppPolicy(
    context: PolicyContext,
    messageType: string
  ): Promise<PolicyDecision> {
   ...
   */
  private async checkTelegramPolicy(
    context: PolicyContext,
    messageType: string
  ): Promise<PolicyDecision> {
    // Check rate limits
   ...
   */
  private async checkTwitterPolicy(
    context: PolicyContext,
    messageType: string
  ): Promise<PolicyDecision> {
   ...
   */
  async getPolicyStatus(context: PolicyContext): Promise<{
    canSend: boolean;
    reason?: string;
   ...
   */
  async selectWhatsAppTemplate(
    context: PolicyContext,
    intent: 'welcome' | 'cart' | 'support' | 'notification' | 'general',
   ...
   */
  async checkContentPolicy(text: string, channel: ChannelType): Promise<{
    allowed: boolean;
    violations?: string[];
  }> {
