...
export const CHANNEL_CAMPAIGN_RULES: Record<string, ChannelCampaignRules> = {
  whatsapp: {
    id: 'whatsapp',
    name: 'WhatsApp',
    icon: 'whatsapp',
 ...
 */
export const getChannelRules = (channelType: string): ChannelCampaignRules | null => {
 ...
 */
export const isCampaignTypeSupported = (
  channelType: string,
  campaignType: 'evergreen' | 'broadcast'
): boolean => {
 ...
 */
export const validateMessageForChannel = (
  channelType: string,
  ...
  if (message.content.length > rules.messageConstraints.maxLength) {
    errors.push(
      `Message exceeds ${rules.messageConstraints.maxLength} character limit for ${rules.name}`
    );
  }
  ...
  if (message.mediaType && !rules.messageConstraints.supportedMediaTypes.includes(message.mediaType)) {
    errors.push(
      `${rules.name} does not support ${message.mediaType}. Supported: ${rules.messageConstraints.supportedMediaTypes.join(', ')}`
    );
  }
  ...
  if (message.buttons && message.buttons.length > 0) {
    if (!rules.messageConstraints.supportsButtons) {
      ...
      errors.push(
        `${rules.name} supports max ${rules.messageConstraints.maxButtons} buttons`
      );
    }
  }
