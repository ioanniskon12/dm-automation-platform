...
export default function KnowledgeBase() {
  ...
  const handleUrlScrape = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/knowledge/scrape`, { url });
      if (response.data.success) {
        setUrl('');
        fetchDocuments();
        updateOnboardingProgress();
  ...
  const handleTextSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/knowledge/text`, {
        title: textTitle,
        content: pastedText,
      });
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (urlChanged) {
        ...
        await axios.delete(`${API_URL}/api/knowledge/${selectedDoc.id}`);

        // Re-scrape with the new URL
        const scrapeResponse = await axios.post(`${API_URL}/api/knowledge/scrape`, { url: editUrl });
        ...
        const response = await axios.put(`${API_URL}/api/knowledge/${selectedDoc.id}`, {
          title: editTitle,
          content: editContent,
        });
  ...
  const deleteDocument = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/api/knowledge/${docToDelete.id}`);
      fetchDocuments();
      updateOnboardingProgress();
  ...
  const fetchInstructions = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/bot-instructions`);
  ...
  const handleSaveInstructions = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/bot-instructions`, {
        purpose: botPurpose,
              ...
              <div className="flex overflow-x-auto border-b border-gray-200 dark:border-gray-700 scrollbar-hide">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`flex-1 min-w-[80px] px-3 md:px-4 py-3 font-semibold text-xs transition-all whitespace-nowrap ${
                      activeTab === tab.id
                      ...
                      <div className="border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
                        {selectedDoc.metadata?.mimetype === 'application/pdf' ? (
                          ...
                          <div className="bg-gray-100 dark:bg-gray-900 p-8 text-center">
                            ...
                            <a
                              href={`${API_URL}/api/knowledge/file/${selectedDoc.id}`}
                              target="_blank"
