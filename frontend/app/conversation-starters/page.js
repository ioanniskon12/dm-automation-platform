...
function ConversationStartersContent() {
  ...
  const [starters, setStarters] = useState([
    { id: 1, question: 'What products do you offer?', clicks: 342, flowId: 'flow-1' },
    { id: 2, question: 'How can I place an order?', clicks: 198, flowId: 'flow-2' },
    { id: 3, question: 'What are your store hours?', clicks: 156, flowId: 'flow-3' },
  ]);
  const [showFlowSelector, setShowFlowSelector] = useState(false);
  const [editingStarterId, setEditingStarterId] = useState(null);
  const [hasChanges, setHasChanges] = useState(false);

  // Mock flows for selector
  const availableFlows = [
    { id: 'flow-1', name: 'Product Catalog Flow', channel: 'Instagram' },
    { id: 'flow-2', name: 'Order Process Flow', channel: 'Instagram' },
    { id: 'flow-3', name: 'Store Hours Info Flow', channel: 'Instagram' },
          ...
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">
                Please select a brand and channel to manage conversation starters
              </p>
              <button
  ...
  const handleAddStarter = () => {
    if (starters.length >= 4) return; // Instagram allows max 4 conversation starters
    const newStarter = {
      id: Date.now(),
  ...
  const handleUpdateQuestion = (id, newQuestion) => {
    setStarters(starters.map(s =>
      s.id === id ? { ...s, question: newQuestion } : s
    ));
    setHasChanges(true);
  };

  const handleSelectFlow = (starterId, flowId) => {
    setStarters(starters.map(s =>
      s.id === starterId ? { ...s, flowId } : s
    ));
  ...
  const handleSave = () => {
    // TODO: API call to save conversation starters
    console.log('Saving conversation starters:', starters);
    setIsEditing(false);
    setHasChanges(false);
  };
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <main className={`px-6 py-8 min-h-screen transition-all duration-300 ${isCollapsed ? 'ml-20' : 'ml-64'}`}>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-bold text-black dark:text-white">Conversation Starters</h1>
                <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-semibold rounded-full">
                  {currentBrand.name} · {currentChannel.type}
                </span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400 ml-8">
                Set up questions that encourage people to start a conversation before any direct message
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Left Panel: Conversation Starters List */}
          <div>
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {starters.map((starter, index) => (
                  <div key={starter.id} className="px-6 py-4">
                      ...
                      <div className="col-span-6">
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) => handleUpdateQuestion(starter.id, e.target.value)}
                            maxLength={80}
                        ...
                        {isEditing && (
                          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {starter.question.length}/80 characters
                          </div>
                        )}
                      </div>
                ...
                {starters.length === 0 && (
                  <div className="px-6 py-12 text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-sm">
                      No conversation starters yet. Add one to get started.
                    </p>
                  </div>
                )}
              </div>

              {/* Add Button (only in editing mode and if less than 4 starters) */}
              {isEditing && starters.length < 4 && (
                <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  >
                    ...
                    Add Conversation Starter ({starters.length}/4)
                  </button>
                </div>
              )}
              ...
              {starters.length === 4 && (
                <div className="px-6 py-3 bg-blue-50 dark:bg-blue-900/20 border-t border-blue-200 dark:border-blue-800">
                  <p className="text-xs text-blue-700 dark:text-blue-300 text-center">
                    Maximum of 4 conversation starters reached
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ul className="text-xs text-purple-800 dark:text-purple-400 space-y-1">
                    <li>• Keep questions short and clear (max 80 characters)</li>
                    <li>• Use questions that cover your most common inquiries</li>
                    <li>• Update starters based on user engagement metrics</li>
                    <li>• Make questions inviting and friendly</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <div className="w-[340px] bg-black rounded-[3rem] p-3 shadow-2xl">
                <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] overflow-hidden h-[680px] flex flex-col">
                  ...
                  {/* Chat Area - Conversation Starters Section */}
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {starters.map((starter) => (
                              <button
                                key={starter.id}
                                className="w-full bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 hover:border-blue-500 dark:hover:border-blue-500 text-left rounded-xl py-3 px-4 text-sm font-medium text-gray-800 dark:text-gray-200 transition-all hover:shadow-sm"
                              >
                                {starter.question}
                              </button>
                            ))}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-blue-100 mt-1">Choose which flow to trigger when this question is selected</p>
                </div>
                <button
              ...
              <div className="p-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-blue-100">
                            Create a new flow for this conversation starter
                          </p>
                        </div>
                      </div>
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {availableFlows.map((flow) => (
                    <button
                      key={flow.id}
                      onClick={() => handleSelectFlow(editingStarterId, flow.id)}
